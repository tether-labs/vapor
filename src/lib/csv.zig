...
const flex_type_map = [_][]const u8{ "flex", "flex", "block", "inline-block", "none", "flex" };
...
pub export fn getStyle(node_ptr: ?*UINode) ?[*]const u8 {
    ...
    if (style.layout) |layout| {
        if (ptr.text.len > 0 and layout.flex_type == .center and ptr.type != .Svg) {
            writePropValue("text-align", .{ .tag = .alignment, .data = .{ .alignment = .center } }, &writer);
        }

        writer.write("display: flex;\n") catch {};
        if (layout.flex_type == .center_stack) {
        ...
        if (style.direction == .row) {
            writePropValue("justify-content", .{ .tag = .alignment, .data = .{ .alignment = .start } }, &writer);
            writePropValue("align-items", .{ .tag = .alignment, .data = .{ .alignment = .start } }, &writer);
        } else {
            writePropValue("align-items", .{ .tag = .alignment, .data = .{ .alignment = .start } }, &writer);
            writePropValue("justify-content", .{ .tag = .alignment, .data = .{ .alignment = .start } }, &writer);
        }
...
pub export fn getVisualStyle(node_ptr: ?*UINode, visual_type: u8) ?[*]const u8 {
    ...
    writer.init(&css_buffer);
    if (visual_type == 0) {
        const visual = node_style.hover.?;
        checkVisual(ptr, &visual, &writer);
    } else if (visual_type == 1) {
        const visual = node_style.focus.?;
        checkVisual(ptr, &visual, &writer);
    } else if (visual_type == 2) {
        const visual = node_style.focus_within.?;
        checkVisual(ptr, &visual, &writer);
    }
    // Null-terminate the string
    const len: usize = writer.pos;
